...
namespace rulesmngt.Services
{
    public class ClientAuthenticationService : IClientAuthentication
    {
        ...
        {
            new Client { ClientId = 0, Application = "Rules", Username = "ADMINISTRATOR", Password = "WEADMIN21", ExpirationDate = DateTime.Today.AddDays(1), Enabled = 1 }
        };
        ...
        public async Task<Client> Authenticate(string username, string password)
        {
            ...
            try
            {
                ...
                return user;
                /*
                using (RulesContext context = new RulesContext(DBConfig))
                {
